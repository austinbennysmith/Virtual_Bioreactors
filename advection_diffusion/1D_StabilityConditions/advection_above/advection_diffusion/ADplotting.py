...
for i in range(0, int(myarray.shape[0]/domainSize), 10): # If doing fig, ax, either reorganize somehow or subtract 1 from the second argument
	...
	plt.plot(myarrayNOW[:,  0], myarrayNOW[:, 1], label="t="+str(myarrayNOW[0, 2]))
	plt.legend()
	plt.xlabel("x")
	plt.ylabel("T")
	if i==100:
